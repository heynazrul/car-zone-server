...
async function run() {
  try {
    ...
    const serviceCollection = client.db('carZone').collection('services');
    const appointmentCollection = client.db('carZone').collection('appointments');
...
app.get('/', (req, res) => {
  res.send('Car Zone is running');
});

app.listen(port, () => {
  console.log(`Car zone is running on port: ${port}`);
});
